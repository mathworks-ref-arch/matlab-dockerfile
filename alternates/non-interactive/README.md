...
```bash
docker build -t matlab-non-interactive:r2023b .
```
...
```bash
docker run --init --rm matlab-non-interactive:r2023b matlab-batch -licenseToken "user@email.com|encodedToken" "rand"
```
...
|---|---|---|
| [MATLAB_RELEASE](#build-an-image-for-a-different-release-of-matlab) | r2023b | The MATLAB release you want to install, in lower-case. For example: `r2022a` |
...
```bash
docker build --build-arg MATLAB_PRODUCT_LIST="MATLAB Statistics_and_Machine_Learning_Toolbox" -t matlab-stats-non-interactive:r2023b .
```
...
```bash
docker build --build-arg MATLAB_INSTALL_LOCATION="/opt/matlab" -t matlab-non-interactive:r2023b .
```
...
- Specify the `-licenseToken` run-arg.
    ```bash
    # Example
    docker run --init --rm matlab-non-interactive:r2023b matlab-batch -licenseToken "user@email.com|encodedToken" "disp('Hello, World.')"
    ```
...
- Specify the `MLM_LICENSE_TOKEN` environment variable.
    ...
    docker run --init --rm -e MLM_LICENSE_TOKEN matlab-non-interactive:r2023b matlab-batch "disp('Hello, World.')"
    ```
...
export MLM_LICENSE_TOKEN="user@email.com|encodedToken"
docker run --init --rm -e MLM_LICENSE_TOKEN matlab-non-interactive:r2023b matlab-batch rand
```
...
export MLM_LICENSE_TOKEN="user@email.com|encodedToken"
docker run --init --rm -v $(pwd):/content -w /content -e MLM_LICENSE_TOKEN matlab-non-interactive:r2023b matlab-batch "myscript"
```
